...
class CosmoResamplingVT(ResamplingVT):
    ...
    def detection_efficiency(self, parameters):
        ...
        samples_in_source = self.redshift_model.detector_frame_to_source_frame(self.data, self.astropy_conv, **parameters)
        jac = self.redshift_model.detector_to_source_jacobian(samples_in_source['redshift'], self.data['luminosity_distance'], **parameters)
