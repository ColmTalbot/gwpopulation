...
from numpyro.distributions import Normal, Gamma, Unit
...
def bilby_to_numpyro_prior(value):
    ...
    mapping = dict(
        ...
        TruncatedGaussian=lambda x: TruncatedNormal(x.mu, x.sigma, low=x.minimum, high=x.maximum),
        TruncatedNormal=lambda x: TruncatedNormal(x.mu, x.sigma, low=x.minimum, high=x.maximum),
...
def construct_numpyro_model(likelihood, priors, likelihood_func=generic_bilby_likelihood_function, **kwargs):
    from bilby.core.prior import DeltaFunction
    priors = priors.copy()
    priors.convert_floats_to_delta_functions()
    delta_fns = {k: v for k, v in priors.items() if isinstance(v, (int, float, DeltaFunction))}
    priors = bilby_to_numpyro_priors(priors)

    def model():
        parameters = sample_prior(priors, delta_fns)
        ln_l = jnp.nan_to_num(likelihood_func(likelihood, parameters, **kwargs), nan=-jnp.inf)
...
def gwpopulation_likelihood_model(
    ...
    variances = deterministic(
        "variances",
        (square_expectations - expectations**2) / (likelihood.samples_per_posterior * expectations**2),
    )
    ...
    if apply_selection:
        ...
        selection_uncertainty = jnp.sum(injection_weights**2) / selector.total_injections**2 - efficiency**2 / selector.total_injections
        selection = deterministic("selection", -jnp.log(efficiency) * likelihood.n_posteriors)
        selection_variance = deterministic(
            "selection_variance",
            selection_uncertainty / efficiency**2 * likelihood.n_posteriors**2,
        )

        total_hypervolume = likelihood.selection_function.surveyed_hypervolume(likelihood.parameters)
        vt = total_hypervolume * efficiency
        _ = sample("rate", Gamma(likelihood.n_posteriors, 1 / vt))
        if predictive_resample:
            posterior_predictive_resample(selector.data, injection_weights, "injections", shape=(likelihood.n_posteriors,), fit_keys=fit_keys)
    else:
        selection = 0
    ...
    if predictive_resample:
        for event in range(likelihood.n_posteriors):
            posterior_predictive_resample(likelihood.data, weights, event, event=event, fit_keys=fit_keys)
    if varmax < np.inf:
        ln_l = jnp.where(
...
def posterior_predictive_resample(data, weights, label, event=None, shape=(), fit_keys=None):
    if event is not None:
        weights = weights[:, event]
    weights /= jnp.sum(weights)
    n_points = int(weights.shape[0])
    idx = jax.random.choice(jax.random.PRNGKey(0), jnp.arange(n_points), p=weights, shape=shape, replace=False)
    if fit_keys is None:
        fit_keys = data.keys()
    for key in fit_keys:
        if event is None:
